...
export const Scroll3DModel: React.FC<Scroll3DModelProps> = ({ activeMode }) => {
  ...
  useEffect(() => {
    ...
    const mats = {
        ...
        wallAccent: new THREE.MeshStandardMaterial({ color: 0xD97706, roughness: 0.7 }), 
        floor: new THREE.MeshStandardMaterial({ color: 0xE7E5E4, roughness: 1 }), 
        desk: new THREE.MeshStandardMaterial({ color: 0xD4A373, roughness: 0.6 }), 
        chair: new THREE.MeshStandardMaterial({ color: 0x334155, roughness: 0.9 }), 
        monitor: new THREE.MeshStandardMaterial({ color: 0x0F172A, roughness: 0.2, metalness: 0.8 }), 
        ...
        plant: new THREE.MeshStandardMaterial({ color: 0x166534, roughness: 1 }), 
    ...
    const buildDesk = (x: number, z: number) => {
        ...
        // Z-Offset increased to 0.04 to fix flickering/Z-fighting permanently
        const screenZ = z - 0.4 + 0.04; 
        desk.add(createBox(0.84, 0.49, 0.005, mats.screen, x, 1.05, screenZ)); 

        desk.add(createBox(0.6, 0.1, 0.6, mats.chair, x, 0.45, z + 0.8));
        desk.add(createBox(0.6, 0.8, 0.1, mats.chair, x, 0.45, z + 1.1));
        desk.add(createBox(0.1, 0.45, 0.1, mats.metal, x, 0, z + 0.8));
        
        desk.add(createBox(0.3, 0.02, 0.2, mats.wallAccent, x + 0.7, 0.85, z + 0.2));
        return desk;
    };
    ...
    // --- Build Building ---
    buildingGroup.add(createBox(15, 1.8, 15, mats.wall, 0, 0, 0));
    buildingGroup.add(createBox(6, 28, 6, mats.core, 0, 0, 0));
    
    for(let i=0; i<9; i++) {
        const h = 2.8;
        const yPos = 1.8 + (i * h);
        buildingGroup.add(createBox(12.5, 0.25, 12.5, mats.floor, 0, yPos, 0));
    ...
    buildingGroup.add(createBox(1, 6, 1, mats.metal, 0, 31, 0));
